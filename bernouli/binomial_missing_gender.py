...
model_path = "./result/binomial" + "_" + time.strftime('%Hh%Mm_%m/%d/%y') + '_' +notice + '_' + str(N_sample) + '_' + str(draws) + '_' + str(tune) + '_' + str(chains) + '/'
...
with pm.Model() as binomial_model:
    ...
    phi_male = a + b * x[idx_missing, 1] + c * 1 + d * x[idx_missing, 2] + e * x[idx_missing, 3] + f * x[idx_missing, 4]
    phi_female = a + b * x[idx_missing, 1] + c * 0 + d * x[idx_missing, 2] + e * x[idx_missing, 3] + f * x[idx_missing,4]
