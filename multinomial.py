...
with pm.Model() as multinomial:
    a = pm.Normal("a", mu=0, sigma=1, shape=3)  # intercepts
    b = pm.Normal("b", mu=0, sigma=1.5, shape=3)  # association of income with choice

    s0 = a[0] + b[0] * X_shared[:, 0]
    s1 = a[1] + b[1] * X_shared[:, 1]
    s2 = np.zeros(800)#a[2] + b[2] * X_train[:, 2]  # pivoting the intercept for the third category
    s = pm.math.stack([s0, s1, s2]).T

    p_ = tt.nnet.softmax(s)
    career_obs = pm.Categorical("career", p=p_, observed=y_train)

    trace_multinomial = pm.sample(draws=1000,
                           tune=100,
                           chains=1,
                           cores=1,
                           init='adapt_diag')
